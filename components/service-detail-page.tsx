    ...
    <main className="service-native">
      <section className="service-native__hero">
        <div>
          ...
          <Link className="button" href="/contact">프로젝트 상담하기 <ArrowRight aria-hidden size={18} /></Link>
        </div>
        <Image src={service.image} alt={service.imageAlt} width={1600} height={1024} priority sizes="(max-width: 900px) 100vw, 52vw" />
      </section>

      <section className="service-native__process" aria-label={`${service.title} 진행 흐름`}>
        <p>{service.title}</p>
      ...
      <section className="service-native__cta">
        ...
        <h2>무엇부터 바꿔야 할지<br />함께 정리해보세요.</h2>
        <Link className="button" href="/contact">무료 사전 진단 신청하기 <ArrowRight aria-hidden size={18} /></Link>
      </section>
    </main>
  );
}
