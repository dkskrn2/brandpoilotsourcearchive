...
import { Article, ChartBar, ChatCircleDots, Gear, House, RocketLaunch, SignOut, SquaresFour } from "@phosphor-icons/react/dist/ssr";
import { logoutAdminAction } from "@/app/admin/auth-actions";

export function AdminSidebar({ active = "dashboard" }: { active?: "dashboard" | "content" | "inquiries" | "analytics" | "brand-pilot" }) {
  return (
    <aside className="admin-sidebar">
      ...
      <nav aria-label="관리자 메뉴">
        ...
        <Link className={active === "brand-pilot" ? "is-active" : ""} href="/admin/brand-pilot" aria-current={active === "brand-pilot" ? "page" : undefined}>
          <RocketLaunch aria-hidden size={20} weight={active === "brand-pilot" ? "fill" : "regular"} /><span>Brand Pilot</span>
        </Link>
