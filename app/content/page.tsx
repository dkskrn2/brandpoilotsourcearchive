...
export default async function ContentPage() {
  ...
  return (
    <main className="content-index">
      ...
      {featured ? <section className="content-featured content-shell">
        <Link href={`/content/${featured.slug}`} className="content-featured__image" aria-label={`${featured.title} 읽기`}>
          <Image src={featured.image} alt={featured.imageAlt} width={1600} height={1024} priority sizes="(max-width: 800px) calc(100vw - 32px), 60vw" />
        </Link>
          ...
          {articles.map((article) => (
            <article key={article.slug}>
              <Link className="content-list__image" href={`/content/${article.slug}`} aria-label={`${article.title} 읽기`}>
                <Image src={article.image} alt={article.imageAlt} width={960} height={640} sizes="(max-width: 700px) 112px, 260px" />
              </Link>
              <div>
