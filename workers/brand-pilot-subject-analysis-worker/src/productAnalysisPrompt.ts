...
export function buildProductAnalysisPrompt(job: SubjectAnalysisJobV2): string {
  ...
  const schema = {
    ...
    productProfile: {
      ...
      productImageCandidates: [{ attachmentId: "uuid", reason: "string" }],
      detailImageCandidates: [{ attachmentId: "uuid", reason: "string" }],
    },
    serviceProfile: null,
    serviceSubtype: null,
    sourceGaps: ["string"],
  ...
  return [
    ...
    "productProfile에서 가격·할인·프로모션, 후기의 반복 만족·불만 패턴, 제품 이미지 후보와 상세 이미지 후보를 구분해 기록한다.",
