...
export function createClient(apiUrl: string, token: string, fetchImpl: typeof fetch = fetch): MarketingClient {
  ...
  return {
    async claim(workerId) { return (await (await request("/worker/ai-content-jobs/marketing/claim", { workerId, leaseSeconds })).json() as { job: MarketingJob | null }).job; },
