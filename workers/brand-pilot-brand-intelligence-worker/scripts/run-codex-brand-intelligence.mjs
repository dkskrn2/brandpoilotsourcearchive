...
if (code !== 0) throw new Error(`brand_intelligence_codex_failed:${code}:${stderr.slice(0, 500)}`);
  ...
  for (const line of value.trim().split(/\r?\n/).reverse()) {
    try {
      ...
      for (const candidate of [parsed.text, parsed.output, parsed.item?.text]) {
        ...
        try { return JSON.parse(candidate); } catch { /* Try another progress event. */ }
      }
      if (parsed.contractVersion === "brand-intelligence-result.v1") return parsed;
    } catch { /* Codex emits JSONL progress events. */ }
  }
