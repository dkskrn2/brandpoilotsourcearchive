...
- 페이지당 최대 50개만 수집한다. 추가 페이지는 MVP에서 호출하지 않는다.
...
첫 수집은 최대 50개이며 화면에서는 20개씩 표시한다. `더 보기`는 DB에 이미 저장된 다음 항목만 표시하며 Meta 추가 페이지를 호출하지 않는다.
