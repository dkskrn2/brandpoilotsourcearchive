...
- 블로그는 실제 HTML과 대표 이미지, 메타 정보를 보여준다.
...
- `workers/brand-pilot-blog-worker`: 블로그 분석, 기획, HTML·메타 정보와 대표 이미지 생성
...
```text
React 생성 마법사
  -> 분석 generation + analyze job 생성
  -> 유형별 워커가 분석 결과와 타깃·소구점 추천 저장
  -> 사용자가 추천을 선택하고 나머지 생성 지시 저장
  -> output + generate jobs 생성
  -> 유형별 워커가 실제 결과 생성
...
분석 시작 API는 DB 트랜잭션 안에서 generation과 analyze job을 함께 만든다. 분석이 완료된 뒤 사용자가 타깃·소구점·레퍼런스와 최종 지시를 확정하면 생성 시작 API가 output 슬롯과 generate job을 함께 만든다. 두 API는 각자의 idempotency key를 사용하며 같은 요청이 다시 들어오면 새 작업을 만들지 않고 기존 generation을 반환한다.
...
- 다운로드는 HTML과 대표 이미지, 전체 ZIP을 제공한다.
...
카드뉴스·마케팅 워커는 기존 이미지 생성 방식과 동일한 Codex `image_gen` 흐름을 각각 구현하되 결과 schema를 공유하지 않는다. 블로그 워커는 구조화된 본문, HTML, 메타 정보와 대표 이미지를 한 작업 안에서 완성한다. 제품 분석과 타깃·소구점 추천은 사용자가 Step 1에서 선택한 콘텐츠 유형의 워커가 `analyze` 작업으로 수행한다. 중앙 API는 콘텐츠 문구, 이미지 장수나 블로그 구조를 대신 결정하지 않는다.
...
- 블로그 1건을 생성해 실제 HTML과 대표 이미지를 확인한다.
