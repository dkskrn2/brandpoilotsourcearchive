...
Wiki 버전 상태는 다음 순서로 전이한다.

```text
collecting -> compiling -> embedding -> validating -> active
                                             -> failed
```
...
4. `어떤 제품이 있나요?`, `어떤 서비스를 제공하나요?`, 제품 상세, 정책, 지식 부족 질문 세트를 비교한다.
5. 근거 정확도와 URL 검증이 통과하면 Growthline만 새 검색을 활성화한다.
6. 신규 브랜드는 최초 Wiki 빌드부터 새 구조를 사용한다.
7. 기존 브랜드는 재크롤링 또는 수동 재생성 시 전환한다.
