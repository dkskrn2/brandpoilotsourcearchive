...
Brand Pilot Managed Content Automation은 사업자와 브랜드 운영자가 자사 URL, 참고 URL, 주제표를 등록하면 매일 콘텐츠 초안을 생성하고, 사용자의 승인 후 Instagram, Threads, Webflow에 자동 게시하는 관리형 콘텐츠 운영 서비스다.
...
| 항목 | 결정 |
|---|---|
| 초기 채널 | Instagram, Threads, Webflow |
...
16. 게시 성공/실패 상태 표시
17. 고객별 채널 연결 정보 저장
18. Webflow 사이트/컬렉션/필드 매핑
19. Instagram 게시용 이미지 렌더링 및 공개 URL 업로드
20. 브랜드 전체 자동 승인 설정
21. 자동 승인 차단 사유 표시
22. 기본 운영 로그
...
- Webflow 글에서는 출처 언급 옵션을 나중에 검토한다.
...
- `auto_approval_blocked`: 자동 승인 조건을 통과하지 못함
- `rejected`: 거절됨
- `regenerating`: 재생성 중
- `regenerated`: 재생성 완료
...
- Webflow부터 품질 기준을 검증하고, Instagram/Threads는 파일럿 연결 계정에서 자동 게시 안정성을 확인한다.
...
- 이미지 렌더링에 실패했다.
- Webflow 필드가 누락됐다.
- Instagram/Threads/Webflow 토큰 또는 권한 문제가 있다.
- 같은 주제 반복 가능성이 높다.
...
Webflow는 초기 자동 게시에 가장 적합하다.

필요한 것:

- Site ID
- Collection ID
- 초기 API token. 이후 OAuth token으로 전환
- 필드 매핑
- slug 생성 정책
- staged item 생성
- publish item 호출
...
- 고객마다 CMS 필드명이 다르다.
- Rich text 변환 품질이 중요하다.
- API 토큰 권한이 부족하면 게시 실패한다.
- Webflow의 예약 발행 상태를 API로 직접 제어하기 어렵기 때문에 우리 서비스가 스케줄러 역할을 해야 한다.
...
- Instagram/Threads/Webflow별 결과물
...
Worker
  ...
  -> upload artifacts
    -> publish Webflow/Instagram/Threads
...
- 초기 site/API token. 이후 OAuth access token
- site id
- collection id
- field mapping
...
5. Worker를 배포한다.
6. Scheduler를 설정한다.
7. Webflow 자동 게시를 먼저 연결한다.
8. Instagram은 Brand_Pilot publish 로직을 고객별 credential 기반으로 바꾼다.
9. Threads는 텍스트 게시부터 붙인다.
...
| Webflow 글 생성 실패 | 필드 매핑 오류 | mapping_required 상태와 운영자 오류 표시 |
...
- Webflow까지 자동 게시되는 최소 운영 루프를 만든다.
...
- 동일 주제를 Instagram/Webflow/Threads로 재가공해서 게시할 수 있다.
...
- 운영자 개입 없이 신규 고객이 최소 Webflow까지 연결하고 게시할 수 있다.
...
- 기본값은 off로 둔다.
- Webflow부터 허용하고 Instagram/Threads는 파일럿 고객에게만 제한한다.
...
- 브랜드 관점으로 재해석
- 외부 URL은 내부 근거로 기록
- Webflow 장문 콘텐츠는 출처 표시 옵션 검토

### 16.5 고객 온보딩 비용

OAuth 연결로 고객 난이도는 낮아지지만, 계정 타입, 권한, Webflow 필드 매핑, 자동 승인 정책 안내 때문에 초기 온보딩 비용은 여전히 존재한다.
...
같은 주제라도 Instagram은 승인, Webflow는 재생성, Threads는 거절될 수 있다.
...
- Phase 1의 현실적인 시작점으로는 좋지만, 제품 전체 포지션은 멀티 채널로 유지한다.
...
3. 주제표 CSV 템플릿을 만든다.
4. 파일럿 고객 후보 3곳을 정한다.
5. Webflow 자동 게시부터 내부 MVP를 만든다.
6. Brand_Pilot Instagram 게시 로직을 고객별 OAuth credential 기반 config로 분리할 설계를 한다.
7. 브랜드 전체 자동 승인 정책과 차단 조건을 Webflow 기준으로 먼저 정의한다.
