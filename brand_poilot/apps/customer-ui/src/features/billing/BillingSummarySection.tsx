...
function formatAmount(amount: number | null) {
  return amount === null
    ? "가격 준비 중"
    : `월 ${new Intl.NumberFormat("ko-KR").format(amount)}원`;
}
...
export function BillingSummarySection({
  loadBillingSummary = api.getBillingSummary,
  ...
  useEffect(() => {
    let active = true;
    void loadBillingSummary(DEMO_BRAND_ID)
      .then((value) => {
        if (active) setSummary(value);
      })
      .catch(() => {
        if (active) {
          setError("결제 정보를 불러오지 못했습니다. 잠시 후 다시 시도해 주세요.");
        }
      });
    return () => {
      active = false;
  ...
  return (
    <section className="content billing-page">
      ...
      {error ? <div role="alert"><Alert title="결제 정보" variant="bad">{error}</Alert></div> : null}
      ...
      {summary ? (
        <div className="billing-sheet">
          <section className="billing-section" aria-labelledby="billing-plan-heading">
            <div className="billing-section-head"><h2 id="billing-plan-heading">청구</h2></div>
            ...
            {!summary.configured ? <p className="billing-note">토스페이먼츠 연동 후 사용할 수 있습니다.</p> : null}
          </section>

          <section className="billing-section" aria-labelledby="billing-history-heading">
            <div className="billing-section-head"><h2 id="billing-history-heading">청구 내역</h2></div>
            {summary.payments.length === 0 ? (
              <div className="billing-empty-row"><ReceiptText size={18} aria-hidden="true" /><span>결제 이력이 없습니다.</span></div>
            ) : (
          ...
          <section className="billing-section" aria-labelledby="billing-profile-heading">
            <div className="billing-section-head"><h2 id="billing-profile-heading">결제 정보</h2><button className="button" type="button" disabled>편집</button></div>
          ...
          <section className="billing-section" aria-labelledby="billing-method-heading">
            <div className="billing-section-head"><h2 id="billing-method-heading">결제 방법</h2><button className="button" type="button" disabled>새로 추가</button></div>
            {summary.paymentMethod ? (
              <div className="billing-method-row"><CreditCard size={20} aria-hidden="true" /><div><strong>{summary.paymentMethod.label}</strong>{summary.paymentMethod.last4 ? <span>•••• {summary.paymentMethod.last4}</span> : null}</div></div>
            ) : (
              <div className="billing-empty-row"><CreditCard size={18} aria-hidden="true" /><span>등록된 결제수단이 없습니다.</span></div>
            )}
            <p className="billing-security-note">카드번호, 유효기간, CVC는 토스페이먼츠에서 직접 입력하며 모종에 저장하지 않습니다.</p>
          </section>
        </div>
      ) : null}
    </section>
