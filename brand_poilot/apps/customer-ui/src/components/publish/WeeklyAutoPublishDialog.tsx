...
export function WeeklyAutoPublishDialog({
  ...
  const limit = usage?.publishing.limit ?? null;
      ...
      >
        ...
        <div className="weekly-auto-publish-dialog__body">
          ...
          <section className="weekly-auto-publish-dialog__section" aria-labelledby="weekly-auto-schedule-title">
            <div className="weekly-auto-publish-dialog__section-heading">
              <div><h3 id="weekly-auto-schedule-title">주간 게시 계획</h3><p>{limit === null ? `주간 게시 계획 ${rows.length}건` : `주간 게시 계획 ${rows.length}/${limit}건`}</p></div>
            </div>
                ...
                return <fieldset className="weekly-auto-publish-day" aria-label={day.label} key={day.dayOfWeek}>
                  ...
                  <div className="weekly-auto-publish-day__rows">
                    {dayRows.map((row, index) => <div className="weekly-auto-publish-row" role="group" aria-label={`${day.name} ${index + 1}번째 게시 일정`} key={row.key}>
                      <label><span className="visually-hidden">{day.name} {index + 1}번째 게시 시간</span><input type="time" step="60" aria-label={`${day.name} ${index + 1}번째 게시 시간`} value={row.time} onChange={(event) => updateTime(row.key, event.target.value)} /></label>
                      <div className="weekly-auto-publish-row__actions">
                        <button className="button icon-button" type="button" aria-label={`${day.name} ${index + 1}번째 일정을 위로 이동`} disabled={index === 0} onClick={() => moveRow(day.dayOfWeek, index, -1)}><ChevronUp size={16} aria-hidden="true" /></button>
                        <button className="button icon-button" type="button" aria-label={`${day.name} ${index + 1}번째 일정을 아래로 이동`} disabled={index === dayRows.length - 1} onClick={() => moveRow(day.dayOfWeek, index, 1)}><ChevronDown size={16} aria-hidden="true" /></button>
                        <button className="button icon-button" type="button" aria-label={`${day.name} ${index + 1}번째 일정 삭제`} onClick={() => deleteRow(row.key)}><Trash2 size={16} aria-hidden="true" /></button>
                      </div>
                    </div>)}
                  </div>
                  <button className="button weekly-auto-publish-day__add" type="button" aria-label={`${day.name} 일정 추가`} disabled={(limit !== null && rows.length >= limit) || dayRows.length >= 24} onClick={() => addRow(day.dayOfWeek)}><Plus size={15} aria-hidden="true" /> 시간 추가</button>
                </fieldset>;
              })}
            </div>
          </section>
        ...
        <footer className="weekly-auto-publish-dialog__footer">
          ...
          <button ref={saveButtonRef} className="button primary" type="button" aria-busy={submitting || saving} disabled={submitting || saving} onClick={() => void save()}>설정 저장</button>
        </footer>
      </FocusTrap>
    </div>
  );
