...
interface Props { brandId: string; gateway: AiContentGateway; draft: AiContentDraft; onBrief(update: GenerationBriefUpdate): void; generationId: string | null; }

export function GenerationPromptStep({ brandId, gateway, draft, onBrief, generationId }: Props) {
  const brief = draft.brief!;
  const [brandColor, setBrandColor] = useState("#0057B8");
  useEffect(() => { let active = true; void gateway.getBrandContext(brandId).then((context: AiContentBrandContext) => { if (active) { setBrandColor(context.brandColor ?? "#0057B8"); if (brief.selectedColor === "#0057B8" && context.brandColor) onBrief({ ...brief, selectedColor: context.brandColor }); } }).catch(() => undefined); return () => { active = false; }; }, [brandId, gateway]);
  const update = (value: Partial<GenerationBrief>) => onBrief({ ...brief, ...value });
  const updateDirection = (index: number, value: string) => { const outputDirections = [...brief.outputDirections]; outputDirections[index] = value; update({ outputDirections }); };
  const applyToAll = () => update({ outputDirections: Array.from({ length: brief.outputCount }, () => brief.outputDirections[0] ?? "") });
  ...
  return <section className="generation-prompt-step"><header className="wizard-section-heading"><div><p className="eyebrow">STEP 5</p><h2>프롬프트와 생성 설정을 확인하세요</h2><p className="wizard-lead">브랜드 대표 색상을 기본값으로 사용하며, 결과별 지시와 참고 이미지를 생성 워커에 함께 전달합니다.</p></div></header>
    ...
    <div className="generation-attachments"><div className="section-heading-inline"><h3>추가 참고 이미지</h3><span>제품·인물·크기 역할을 선택할 수 있습니다.</span></div><AiContentAttachmentUploader gateway={gateway} brandId={brandId} generationId={generationId} attachments={brief.attachments} totalAttachmentCount={activeAttachmentCount} allowedRoles={["product", "person", "scale", "visual_reference"]} onChange={updateAttachments} /></div>
