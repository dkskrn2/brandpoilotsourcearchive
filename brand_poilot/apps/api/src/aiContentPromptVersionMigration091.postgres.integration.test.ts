...
const applicationRole = "ai_content_prompt_v4_application";
const applicationPassword = "ai-content-prompt-v4-application-test";
...
it("allows exact v2, v3, and v4 proposal lineage without widening application privileges", async () => {
  ...
  try {
        ...
        constraint ai_content_proposal_job_contracts_versions_check check(
          ...
          and (
            (proposal_prompt_version='proposal.writer.v2'
              and contract_source_sha256='${v2SourceHash}'
              and catalog_sha256='${v2CatalogHash}')
            or (proposal_prompt_version='proposal.writer.v3'
              and contract_source_sha256='${v3SourceHash}'
              and catalog_sha256='${v3CatalogHash}')
          )
        )
      );
    ...
    `);
    await insertProposalContract(administrator, "proposal.writer.v2", v2SourceHash, v2CatalogHash);
    await insertProposalContract(administrator, "proposal.writer.v3", v3SourceHash, v3CatalogHash);
    await insertPromptBinding(administrator, "proposal.writer.v2", v2SourceHash);
    await insertPromptBinding(administrator, "proposal.writer.v3", v3SourceHash);
    ...
    await insertProposalContract(application, "proposal.writer.v2", v2SourceHash, v2CatalogHash);
    await insertProposalContract(application, "proposal.writer.v3", v3SourceHash, v3CatalogHash);
    await insertProposalContract(application, "proposal.writer.v4", v4SourceHash, v4CatalogHash);
    await insertPromptBinding(application, "proposal.writer.v2", v2SourceHash);
    await insertPromptBinding(application, "proposal.writer.v3", v3SourceHash);
    await insertPromptBinding(application, "proposal.writer.v4", v4SourceHash);

    await expect(insertProposalContract(
      application, "proposal.writer.v4", v3SourceHash, v3CatalogHash,
    ...
    expect(stored.rows).toEqual([
      { proposal_prompt_version: "proposal.writer.v2", count: 2 },
      { proposal_prompt_version: "proposal.writer.v3", count: 2 },
