...
export function toBrandIntelligenceV1Compatibility(
  result: BrandIntelligenceResult,
): BrandIntelligenceResultV1 {
  if (result.contractVersion === "brand-intelligence-result.v1") return result;
  return {
    contractVersion: "brand-intelligence-result.v1",
    companyOverview: result.companyOverview ?? "",
    businessDescription: result.businessDescription ?? "",
    primaryCategory: result.primaryCategory ?? { code: null, name: "" },
    subcategories: result.subcategories,
    primaryTarget: result.primaryTarget ?? "",
