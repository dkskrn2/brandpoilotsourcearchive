...
Require JSON `applied` to contain exactly `074_ai_content_maintenance_write_fence.sql` and no secret; the trap removes both container-readable copies on success, error, or interruption. If a database CA file is required, add it to the same exact-path trap, mount a UID-readable read-only copy, and point the existing TLS file variable to its container path; never put CA/URL/authorization contents in `--env`. Query `schema_migrations` to prove `074` present/`075` absent and verify the new trigger/role catalog from every real role. The serialized provider-admin workflow revokes migration membership and seals zero-membership/session evidence. Leave maintenance disabled during ordinary F health verification. Do not enable either migrated writer.
