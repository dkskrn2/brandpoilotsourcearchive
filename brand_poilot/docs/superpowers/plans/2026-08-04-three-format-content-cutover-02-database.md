...
Tests must prove default-off compatibility; HTTP `503 ai_content_maintenance` before mutation; the same guard as the first statement in the owning transaction; unchanged row counts for create, proposal-create, select, start, retry, regenerate, automated-run, attachment/render/finalization, download, and internal mutations; and direct stale-process writes blocked at the database root. A download request must fail before remote asset fetch or ZIP construction, and `runDailyGeneration()` must check maintenance before its first brand query even when invoked outside HTTP. Build the exact current AI-content execution-relation catalog from the Phase 2 deletion-graph enumerator, including shared topic/draft/output/publication/artifact/automation relations mutated by automated content. Every catalogued relation must have either the common whole-relation fence or an explicitly tested shared-table row classifier; classifiers inspect `NEW` for INSERT, `OLD` for DELETE, and both `OLD` and `NEW` for UPDATE, block every operation or reclassification on an in-scope row, and preserve unrelated rows. In particular the scheduled completion bridge's `storage_artifacts` and `publish_queue` writes, `publish_attempts` linked through an AI-content `publish_queue`/`channel_outputs` row, and `automation_runs` rows with `run_type='daily_generation'` require explicit classifiers. An uncatalogued writable edge or classifier that permits a fresh or existing legacy automated row mutation fails `074`. Docker/release tests require the fence API image built from this commit to contain the migration runner plus migrations through exact `074_ai_content_maintenance_write_fence.sql`, and to fail if `075_ai_content_three_format_cutover.sql` is present.

The F runner must already support the signed two-stage `074` bootstrap protocol. Stage one validates the unused authorization file/image/migration/role-catalog hashes against canonical hashes recomputed from the live database, verifies `session_user` is the migration login, uses `SET LOCAL ROLE` only for the no-login schema owner, and applies exact `074` without executing `CREATE EVENT TRIGGER`, `ALTER EVENT TRIGGER`, or any provider ownership transfer. Exactly one migration may be pending and it must be `074`; any earlier missing migration, existing/pending `075`, or extra pending file fails before mutation. The canonical role catalog must seal both directions of every relevant membership edge, including all direct/indirect members that can `SET ROLE` to schema-owner/operator/migration/cleanup, and the PostgreSQL 16 `SET`/`INHERIT`/`ADMIN` options: the sole owner-capable edge is migration→schema-owner with `SET TRUE`, `INHERIT FALSE`, `ADMIN FALSE`, and no other role may assume any privileged identity. It independently reads and seals a canonical interim post-`074` security catalog containing the exact fence/catalog/control function definitions, owners, `SECURITY DEFINER` state, fixed `search_path`, ordinary trigger relation/name/tgtype/function/ENABLE ALWAYS set, control-table owners/ACLs, and complete PUBLIC/app/operator/migration/cleanup function/table grant equality; calling the migration-owned verifier alone is insufficient. It emits and durably records a sealed provider-admin install request bound to the pre-`074` role/object hashes and this interim security-catalog hash. Stage two consumes a one-shot attestation produced only by the serialized Supabase platform-`postgres` workflow: in one provider transaction that workflow must transfer the exact enforcement bundle defined below to platform `postgres`, fix and verify its ACLs, and create/enable exactly the one approved `074` DDL event trigger. It must prove the final provider-owned security catalog, function identities/body hashes, owners/ACLs, event-trigger name/event/tag filter/definition/enabled state, full before/after event-trigger catalog hashes, source authorization, and non-replay. The runner durably consumes the authorization/attestation, rejects any unapproved ownership/ACL/function/event-trigger catalog delta including a differently named wrapper-function trigger, independently recomputes the full live role/object/final-security/event-trigger catalogs, and emits a sealed revocation request only when every hash and exact property still matches. Exact crash recovery may return the already sealed identical result without repeating a provider mutation only after performing the same fresh live recomputation and recomputing the canonical stored authorization, install request, full signed provider-attestation envelope, and revocation request JSON/hash pairs from their fields. It reconstructs the expected revocation payload from the sealed authorization/install/attestation rather than trusting stored JSON; altered catalog/evidence, JSON/hash divergence, or unconsumed replay fails. It must reject an arbitrary migration or provider DDL, direct owner/provider credentials, missing or altered provider evidence, stale/replayed authorization, any application membership in the schema-owner/operator/migration/cleanup roles, any inbound member of a privileged role, or any other role edge. Bootstrap state binds all five exact live roles; `prepare_ai_content_cutover` must accept only those sealed roles, so the application identity can never be substituted as the migration bypass identity. Task 3 extends this same runner with the prepared-row/token-bound `075` branch; it does not retrofit F later, and `075` is forbidden until the sealed provider-owned enforcement attestation verifies.

Both bootstrap authorization and provider attestation use distinct Ed25519 signing identities. Their private keys exist only in the serialized provider workflow and never enter F/C images, release manifests, host runner state, environment variables, logs, or evidence. The runner receives only read-only PEM/SPKI public-key files, pins the expected key IDs and SHA-256 public-key fingerprints, and accepts only Ed25519 plus the exact canonical envelope and base64 signature shape. The runner has no signing helper or private-key input. Tests prove provider-private-key signing/public-key verification, rejection of wrong key/algorithm/signature/fingerprint/key ID/envelope, and that a runner holding only public material cannot mint either authorization or attestation.
...
Make the database boundary role-safe. The no-login schema owner continues to own customer-execution relations and ordinary schema objects needed by `075`, but it must not own or be a member of the owner of the immutable enforcement bundle. In the provider transaction, platform `postgres` becomes owner of `enforce_ai_content_ddl_allowlist()`, `enforce_ai_content_write_fence()`, `assert_ai_content_writable()`, `ai_content_cutover_bypass_allowed()`, `verify_ai_content_write_fence_catalog()`, `ai_content_fence_trigger_name(text)`, every maintenance/prepare/transition/immutability control function, `ai_content_cutovers`, `ai_content_cutover_status_events`, `ai_content_maintenance_state`, `ai_content_bootstrap_state`, `ai_content_ddl_allowlist`, `ai_content_write_fence_catalog`, and the DDL event-trigger object. Provider-owned `SECURITY DEFINER` functions have a fixed `search_path` and only narrowly reviewed app/operator/migration grants. Ordinary row-trigger objects remain attached to schema-owner-owned execution tables, but the unmodifiable provider-owned DDL guard rejects their disable/drop or any unapproved replacement. All five cutover roles are exact `NOSUPERUSER NOBYPASSRLS NOCREATEDB NOCREATEROLE NOREPLICATION`; the schema owner is `NOLOGIN`, and the migration login is additionally `NOINHERIT`. The canonical role catalog includes and hashes every one of those attributes and rejects a missing, non-boolean, or elevated value. This is the mandatory boundary for ROLE, DATABASE, replication, and other shared-object commands that PostgreSQL event triggers do not cover. The runtime application role is also a non-owner with no membership in the migration/operator/cleanup roles. Revoke application/PUBLIC direct DML on every provider-owned control relation; grant the app only minimum reads and `EXECUTE` on `assert_ai_content_writable()`. `074` fails if app, migration, operator, cleanup, or schema-owner roles can replace a guard/verifier function, alter the allowlist or bootstrap/maintenance/cutover state outside a narrow provider-owned transition function, or alter/disable a fence trigger. App-role integration fixtures attempt state tampering and direct SQL DML on every catalogued relation during maintenance; all fail with unchanged rows. The scoped `075` migration role plus exact transaction-local cutover/token is the sole general fence bypass and is separately proven to work.

DDL ownership is explicit: the migration login is `NOINHERIT` and normally has no effective owner privileges. It has revocable `SET ROLE` membership in the no-login schema owner. The restricted database operator cannot grant roles; only the serialized provider-admin role tool may activate/revoke that one exact membership from a signed migration request, and it cannot grant any other edge. Separately, its Supabase platform-`postgres` mode is a closed one-shot operation that can install or verify the exact `074` enforcement bundle, or marker-absent recover only that exact partially installed bundle. The install operation has a fixed ownership-transfer/ACL/function/event-trigger manifest and exposes no arbitrary SQL or general provider-admin action.

Migration `074` has a separate bootstrap authorization because no cutover row/maintenance token exists yet: exact `074` migration ID and source SHA-256, exact F API image digest/source label, marker absence, verified role-catalog/bootstrap attestation, exact interim and final enforcement manifests, and one unused Ed25519-signed request. The provider-admin tool grants membership; the runner verifies `session_user=migration_role`, uses `SET LOCAL ROLE schema_owner`, applies only exact `074`, and commits the interim schema-owner-owned bundle plus ordinary triggers without any provider-only ownership/event-trigger action. The runner emits a sealed install request only after the interim catalog verifies. The serialized provider-admin tool then authenticates as Supabase platform `postgres`, validates the same authorization/image/migration/role/object/interim hashes, and in one transaction transfers the exact enforcement bundle to `postgres`, fixes its exact ACLs, and creates/enables the reviewed DDL event trigger pointing at the now-provider-owned guard. It seals canonical final catalog evidence proving every required owner/ACL/body hash and event-trigger property. The runner consumes that Ed25519-signed evidence using only the pinned provider public key, independently verifies the final live catalog read-only, and only then emits the membership-revocation request. Any other migration/body/image/function/trigger/DDL/provider action, missing evidence, or reused request fails. Marker-absent recovery may revert only the exact partially installed bundle matching the authorization and sealed interim catalog; it may not mutate any other object.

For `075`, the platform-`postgres`-owned `SECURITY DEFINER` DDL guard must not use inner `current_user` as the caller identity because PostgreSQL changes it to the function owner. It checks `session_user=migration_role` and binds the outer effective `SET LOCAL ROLE schema_owner` through PostgreSQL's role setting (`current_setting('role')`) after the exact membership/options catalog has proved that only the migration login can select schema owner. It also checks maintenance/status, exact transaction-local cutover/token, migration ID, and the reviewed DDL object/command allowlist. Before accepting `075`, the runner requires the sealed non-replayed `074` provider attestation and independently verifies the entire provider-owned enforcement catalog, not only the trigger object. It executes body, final catalog/hash assertions, marker, and status event in the same transaction; any unexpected DDL/object/catalog delta aborts. Before activation, without token, after revoke, and after marker, migration-role DDL/trigger-disable attempts fail; neither migration nor schema owner can replace either guard, modify control state/allowlist, or alter/disable the provider-owned event trigger. Crash recovery first revokes stale membership or resumes only the same signed runner identity; it never leaves an owner-capable login available. After `075`, close/terminate migration sessions, remove URL/token files, revoke membership, and rotate/`NOLOGIN` the temporary migration login before customer runtime starts. An env-gated real-PostgreSQL integration harness—not PGlite alone—must first prove one exact token-bound allowlisted DDL succeeds as migration→schema-owner, then prove a rogue inbound schema-owner member and migration→schema-owner attempts to replace both guards/verifiers, mutate allowlist/bootstrap/maintenance/cutover state, disable/drop ordinary fence triggers, or execute arbitrary/out-of-window DDL all fail or roll back with unchanged catalog/data. It proves all 43 triggers map exactly to one of the eight reviewed row-classifier branches, benchmarks representative bulk DML for every classifier branch with maintenance off and on (including the expensive `publish_queue`/`publish_attempts` joins), records each baseline/threshold plus the maximum ratio, and blocks production on a material regression. PGlite remains a fast structural test only.
...
Before any `075` transaction, require a separate signed provider-admin allowlist installation contract. Its closed request binds one unused request ID, cutover ID, exact `075` migration ID/source SHA, exact sorted rows `{commandTag,objectIdentityPattern}`, their canonical SHA-256, authorization/provider key IDs, issue/expiry times, and the already sealed `074` enforcement-catalog hash. The platform-`postgres` action may only lock `ai_content_ddl_allowlist`, require marker absence and an empty-or-exact table, insert that exact row set atomically, and return a separately signed attestation containing before/after count/hash and request hash. Exact crash replay after a committed insert returns the same attestation; a changed/reused request, partial/extra row, marker presence, or stale authorization fails closed. The runner has no DML grant on this table: it verifies both signatures, consumes the one-shot evidence, freshly recomputes exact live row count/hash, and refuses `075` before membership activation or SQL execution on any mismatch. RED tests cover missing attestation, forged/stale/replayed request, committed-response-loss recovery, partial/extra rows, and a harness-only direct `postgres` insert that cannot satisfy the production contract.
