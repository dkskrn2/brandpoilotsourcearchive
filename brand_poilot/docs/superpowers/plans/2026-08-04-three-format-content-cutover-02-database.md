...
The F runner must already support the signed two-stage `074` bootstrap protocol. Stage one validates the unused authorization file/image/migration/role-catalog hashes, verifies `session_user` is the migration login, uses `SET LOCAL ROLE` only for the no-login schema owner, and applies exact `074` without executing `CREATE EVENT TRIGGER` or `ALTER EVENT TRIGGER`. It verifies the schema-owner-owned trigger function, ordinary fence triggers, allowlist, and catalog, then emits a sealed provider-admin install request bound to their exact hashes. Stage two consumes a one-shot attestation produced only by the serialized Supabase platform-`postgres` workflow: that workflow may create/enable exactly the one approved `074` DDL event trigger, pointing at the exact schema-owner-owned function, and must prove its name, definition, function identity/hash, `postgres` ownership, enabled state, source authorization, and non-replay. The runner then independently verifies the installed trigger/catalog read-only and emits a revocation request. It must reject `075`, an arbitrary migration or provider DDL, direct owner/provider credentials, missing or altered provider evidence, stale/replayed authorization, or any other role edge. Task 3 extends this same runner with the prepared-row/token-bound `075` branch; it does not retrofit F later, and `075` is forbidden until the sealed event-trigger attestation verifies.
