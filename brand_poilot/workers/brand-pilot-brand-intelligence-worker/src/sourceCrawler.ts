...
export interface SafeCrawlResource {
  finalUrl: string;
  httpStatus: number;
  contentType: string;
  body: Uint8Array;
...
function isUnsafeIpv6(address: string): boolean {
  ...
  return /^(?:fc|fd|fe[89ab]|fec|fed|fee|fef|ff)/.test(normalized)
...
async function resolveHostnameWithSignal(
  ...
  const cancelled = new Promise<never>((_resolve, reject) => {
    abort = () => reject(signal.reason ?? new Error("brand_analysis_cancelled"));
    signal.addEventListener("abort", abort, { once: true });
  });
  try {
    return await Promise.race([resolveHostname(hostname), cancelled]);
  } finally {
...
export async function assertSafeCrawlUrl(
  value: string,
  {
    ...
    signal,
  }: { resolveHostname?: HostnameResolver; signal?: AbortSignal } = {}
) {
  let parsed: URL;
  try {
  ...
  try {
    addresses = await resolveHostnameWithSignal(parsed.hostname, resolveHostname, signal);
  } catch {
    if (signal?.aborted) throw signal.reason ?? new Error("brand_analysis_cancelled");
...
async function resolveSafeCrawlTarget(
  value: string,
  resolveHostname: HostnameResolver,
  signal?: AbortSignal,
) {
  const parsed = await assertSafeCrawlUrl(value, { resolveHostname, signal });
  const addresses = isIP(parsed.hostname)
    ? [{ address: parsed.hostname, family: isIP(parsed.hostname) }]
    : await resolveHostnameWithSignal(parsed.hostname, resolveHostname, signal);
  if (addresses.length === 0 || addresses.some(({ address }) => isUnsafeIpAddress(address))) {
    throw new Error("crawl_url_unsafe_address");
  }
...
export async function crawlSourceUrl(
  ...
  const resource = await fetchSafeCrawlResource(url, {
    fetcher,
    timeoutMs,
    resolveHostname,
    maxResponseBytes,
  ...
  if (resource.httpStatus < 200 || resource.httpStatus >= 300) {
    throw new Error(`HTTP ${resource.httpStatus}`);
  }
  if (resource.contentType
    && !resource.contentType.includes("text/html")
    && !resource.contentType.includes("application/xhtml+xml")
    && !resource.contentType.includes("application/xml")
    && !resource.contentType.includes("text/xml")
    && !resource.contentType.includes("text/plain")) {
    throw new Error(`Unsupported content type: ${resource.contentType}`);
  }
  const rawText = new TextDecoder().decode(resource.body);
  const snapshot = extractPageSnapshot(rawText, resource.finalUrl);
  return {
    ...snapshot,
    canonicalUrl: snapshot.canonicalUrl ?? resource.finalUrl,
    httpStatus: resource.httpStatus,
    rawText,
  };
}

export async function fetchSafeCrawlResource(
  url: string,
  {
    fetcher = fetch,
    timeoutMs = 15_000,
  ...
  try {
    ...
    for (let redirectCount = 0; redirectCount <= 3; redirectCount += 1) {
      const resolved = await resolveSafeCrawlTarget(
        targetUrl,
        resolveHostname,
        controller.signal,
      );
