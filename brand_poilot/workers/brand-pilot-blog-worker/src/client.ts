...
export function createClient(apiUrl: string, token: string, fetchImpl: typeof fetch = fetch): BlogClient {
  ...
  return {
    async claim(workerId) { const job = (await (await request("/worker/ai-content-jobs/blog/claim", { workerId, leaseSeconds })).json() as { job: BlogJob | null }).job; if (job) claimedWorkers.set(job.id, workerId); return job; },
