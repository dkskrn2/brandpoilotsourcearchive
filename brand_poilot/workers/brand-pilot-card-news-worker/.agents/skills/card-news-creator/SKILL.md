...
- Do not create, render, or upload images. Do not use file, shell, or image tools. Use the browser only when the supplied prompt identifies a `topic_url` and explicitly requests direct source reading; otherwise do not use web tools.
...
- Use only the supplied brand, product facts, evidence, reference text, selected proposal context, and—only for `topic_url` work—the directly opened source article.
