...
async function main() {
  ...
  const planner = createCommandRunner(
    process.env.CARD_NEWS_CODEX_PLAN_COMMAND ?? "node scripts/run-codex-card-manuscript-plan.mjs --job \"{{jobFile}}\" --output \"{{outputDir}}\"",
  ...
  if (mode === "watch") {
    const pollMs = contentWorkerPollDelayMs(process.env.CARD_NEWS_WORKER_POLL_MS, 10_000);
