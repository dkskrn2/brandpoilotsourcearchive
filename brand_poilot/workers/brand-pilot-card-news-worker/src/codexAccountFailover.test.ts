...
async function fixture() {
  const root = await mkdtemp(path.join(tmpdir(), "card-news-account-failover-"));
  roots.push(root);
  ...
  await writeFile(script, [
    ...
    'await writeFile(new URL("card-manuscript-plan.json", `file://${output.replaceAll("\\\\", "/")}/`), JSON.stringify({ ok: true }));',
  ].join("\n"));
...
describe("card-news Codex account failover", () => {
  it("replays the frozen prompt under secondary after primary usage exhaustion", async () => {
    ...
    const result = await runner.run({} as never, "frozen card-news prompt");
    const attempts = (await readFile(test.attempts, "utf8")).trim().split("\n").map((line) => JSON.parse(line));

    expect(attempts).toEqual([
      { home: test.pool.profiles[0]?.home, prompt: "frozen card-news prompt" },
      { home: test.pool.profiles[1]?.home, prompt: "frozen card-news prompt" },
    ]);
