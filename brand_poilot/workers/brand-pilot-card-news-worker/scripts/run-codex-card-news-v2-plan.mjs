...
export function buildCodexPrompt(prompt) {
  return `${prompt}\n\n제공된 고정 JSON만 사용하고 파일이나 웹을 조회하지 마세요. `
...
export function buildCodexArgs(outputDir) {
  ...
  return [
    ...
    "-c",
    'permissions.planner.filesystem={":minimal"="read","/codex"="deny","/codex-accounts"="deny",":workspace_roots"={"."="deny"}}',
    "-c",
...
export async function main() {
  ...
  if (exitCode !== 0) throw new Error(`codex_card_news_v2_plan_failed:${exitCode}`);
}
