...
export function codexSpawnOptions(outputDir, source = process.env) {
  return { cwd: outputDir, env: codexChildEnv(source), stdio: ["pipe", "inherit", "inherit"], shell: false };
}
...
export function buildCodexArgs(outputDir) {
  return [
    ...
    "-c", 'permissions.planner.filesystem={":minimal"="read","/codex"="deny",":workspace_roots"={"."="deny"}}',
    ...
    "--output-schema", path.resolve(import.meta.dirname, "marketing-plan-v2.schema.json"),
    "--output-last-message", path.join(outputDir, "marketing-plan.json"), "-C", outputDir, "-",
  ];
}
...
export async function main() {
  ...
  if (exitCode !== 0) throw new Error(`codex_marketing_v2_plan_failed:${exitCode}`);
}
