...
async function expectFailedWorkspaceCleanup(
  ...
  const probeDir = await mkdtemp(path.join(os.tmpdir(), "marketing-cleanup-probe-"));
  ...
  try {
    ...
    await expect(runner.run({ id: "cleanup-job" } as never, "prompt"))
      .rejects.toThrow(mode === "timeout"
        ? /codex_marketing_(timeout|failed)/
        : "codex_marketing_failed:7");
...
describe("marketing production runtime", () => {
  it("emits production JavaScript and imports the shared runtime package", async () => {
    ...
    expect(worker).toMatch(/path\.join\(os\.tmpdir\(\),\s*"brand-pilot-marketing-"/);
  });

  it("runs Codex in the job workspace with a secret-free environment", async () => {
    const runnerUrl = new URL("../scripts/run-codex-marketing.mjs", import.meta.url).href;
  ...
  it("stages the marketing skill and removes only image sessions created by that job", async () => {
    ...
    await writeFile(probeFile, [
      ...
      "await access(new URL('./.agents/skills/marketing-creative/SKILL.md', `file://${outputDir}/`));",
    ...
    try {
      ...
      await expect(access(path.join(output.outputDir, ".agents", "skills", "marketing-creative", "SKILL.md")))
