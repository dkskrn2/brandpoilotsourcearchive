...
export interface DmPoolOptions {
  max?: number;
...
export function resolveDmWorkerDatabaseConfig(env: NodeJS.ProcessEnv = process.env) {
  const connectionString = env.DM_WORKER_DATABASE_URL?.trim();
  ...
  return {
    ...
    } satisfies DmPoolOptions,
  };
}

export function resolveDmPoolConfig(
