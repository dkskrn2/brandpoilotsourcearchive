...
export function buildReelPlanPrompt(
  ...
  const outputShape = {
    ...
    scenes: Array.from({ length: assetCount }, (_, offset) => ({
      ...
      keyVisual: { type: "none | number | before_after | comparison | steps | quote | related_facts", entries: [] },
      supportingTexts: [],
      footnote: null,
  ...
  return [
    ...
    "각 scene에서 반환 가능한 키는 index, editorialRole, purpose, coreMessage, headline, keyVisual, supportingTexts, footnote, visualThesis, layoutArchetype, evidenceIds, productImageAssetIds, avatarImageAssetIds뿐입니다.",
    ...
    "제출 전 keyVisual.type별 고정 role 문자열과 entry 수를 다시 확인하고, 분모가 다른 독립 Claim을 comparison으로 제출하지 마세요.",
