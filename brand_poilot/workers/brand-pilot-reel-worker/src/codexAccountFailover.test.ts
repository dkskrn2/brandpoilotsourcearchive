...
async function fixture() {
  const root = await mkdtemp(path.join(tmpdir(), "reel-account-failover-"));
  roots.push(root);
  ...
  await writeFile(script, [
    ...
    'await writeFile(path.join(output, "reel-plan.json"), JSON.stringify({ ok: true }));',
  ].join("\n"));
  return {
    attempts,
...
describe("reel Codex account failover", () => {
  it("replays the frozen prompt under secondary after primary usage exhaustion", async () => {
    const test = await fixture();
    const runner = createCommandRunner(test.command, 10_000, { accountPool: test.pool });

    const result = await runner.run({} as never, "frozen reel prompt");
    const attempts = (await readFile(test.attempts, "utf8")).trim().split("\n").map((line) => JSON.parse(line));

    expect(attempts).toEqual([
      { home: test.pool.profiles[0]?.home, prompt: "frozen reel prompt" },
      { home: test.pool.profiles[1]?.home, prompt: "frozen reel prompt" },
    ]);
