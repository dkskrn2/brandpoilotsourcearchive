...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--contract-version", choices=("worker-reel.v3", "studio-reel.v3"), default="worker-reel.v3")
...
def main() -> None:
    ...
    is_studio_v3 = args.contract_version == "studio-reel.v3"
    if not isinstance(scenes, list) or (not is_studio_v3 and len(scenes) != 1) or (is_studio_v3 and not 1 <= len(scenes) <= 5):
    ...
    duration = args.seconds_per_scene * len(scene_paths) if is_studio_v3 else args.seconds_per_scene * len(scene_paths) - args.fade_seconds * (len(scene_paths) - 1)
    ...
    for index in range(1, len(scene_paths)):
        ...
        offset = index * args.seconds_per_scene if is_studio_v3 else index * (args.seconds_per_scene - args.fade_seconds)
        filters.append(
