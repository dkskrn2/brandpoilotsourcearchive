...
import { cloneManualBlogImageJobV2, cloneManualImageJobV2 } from "../test/fixtures/manualRender.js";
...
describe("V3 single asset renderer", () => {
  ...
  it("stages the full immutable v2 context and every frozen attachment as read-only optional references", async () => {
    ...
    const renderer = createAiContentAssetRenderer({ workerRoot, readOwned, runChild });
  ...
  it("rejects a blog placeholder binding mismatch before invoking the image CLI", async () => {
    ...
    const runChild = vi.fn();
    const renderer = createAiContentAssetRenderer({ workerRoot, readOwned: vi.fn(), runChild });
  ...
  it("rejects staged checksum mismatches before starting the child", async () => {
    const input = manualBlogJob();
    const runChild = vi.fn();
    const renderer = createAiContentAssetRenderer({ workerRoot, readOwned: vi.fn(async () => Buffer.from("wrong")), runChild });
