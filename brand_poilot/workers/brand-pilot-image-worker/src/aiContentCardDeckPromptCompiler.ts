...
import type { AiContentCardDeckImageAssetPayloadV1 } from "./aiContentCardDeckRenderContract.js";
...
function referencedEditorialText(
  text: string,
  scene: AiContentCardDeckImageAssetPayloadV1["cardDeckCurrentScene"]["scene"],
): string {
  const tokens = [
...
function referencedEditorialValue(
  ...
  if (value !== null && typeof value === "object") {
    return Object.fromEntries(Object.entries(value).map(([key, entry]) => [
      key,
      referencedEditorialValue(entry, scene),
    ]));
  }
  return value;
}

export function compileAiContentCardDeckRenderPrompt(
  payload: AiContentCardDeckImageAssetPayloadV1,
  staged: StagedAiContentAssetInputs,
): string {
  const deck = payload.cardDeckContract.plan;
  const scene = payload.cardDeckCurrentScene.scene;
  return [
    section("GLOBAL VISUAL SYSTEM", {
      deckNarrative: referencedEditorialValue(deck.deckNarrative, scene),
      visualSystem: referencedEditorialValue(deck.visualSystem, scene),
    }),
    section("SCENE PURPOSE", {
      editorialRole: scene.editorialRole,
      compatibilityRole: payload.cardDeckCurrentScene.compatibilityRole,
    ...
    section("REFERENCE FILES", {
      ...
      cardDeckEditorialPlan: "inputs/card-deck-editorial-plan.json",
      cardDeckCurrentScene: "inputs/card-deck-current-scene.json",
      productImages: staged.productImages,
      styleImages: staged.styleImages,
      references: staged.references,
      attachments: staged.attachments,
    }),
    section("RENDERING RULES", [
      "Create exactly one final publish-ready square Instagram card as a PNG.",
      ...
      "The current asset copy is a validation projection; the Deck scene is the sole semantic source.",
      "Do not display coreMessageNonDisplay as additional copy.",
      "Do not create a collage, multiple cards, alternatives, logos, watermarks, or unsupported facts.",
