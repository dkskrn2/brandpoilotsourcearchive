...
test("074 real PostgreSQL provider-owned bundle blocks migration-to-schema-owner exploits and measures bulk overhead", {
  ...
  try {
    ...
    const cutover075ProviderFunctionDefinitions = [
      ...
      `create function public.lock_ai_content_fixed_input_sources(uuid,uuid,uuid,uuid,uuid,uuid[],uuid[])
       returns boolean language sql security definer set search_path=pg_catalog,public as $$ select true $$`,
    ];
    ...
    const cutover075CustomerTriggerFunctions = [
      ...
      "enforce_ai_content_three_format_identity",
    ];
    ...
    const cutover075CustomerCallableFunctions = [
      ...
      "ai_content_generation_input_v3_is_valid(jsonb)",
      "ai_content_plan_v2_is_valid(jsonb)",
      "ai_content_manifest_v3_is_valid(jsonb)",
    ];
    ...
    const cutover075ExistingCustomerCallableFunctions = [
      "start_ai_content_orchestration(uuid,uuid,uuid,jsonb,jsonb,uuid)",
      "select_ai_content_proposal(uuid,uuid,uuid,uuid)",
    ];
    for (const identity of cutover075ExistingCustomerCallableFunctions) {
      await client.query(`alter function public.${identity} owner to content_schema_owner`);
    }
    const cutover075CustomerInternalFunctions = [
      "ai_content_cutover_storage_value_to_path(text)",
      "ai_content_cutover_storage_candidates()",
      "ai_content_cutover_target_ids()",
    ];
    for (const identity of cutover075CustomerInternalFunctions) {
      await client.query(
        `create function public.${identity} returns text
         language sql set search_path=pg_catalog,public,pg_temp as $$ select 'ok'::text $$`,
      );
      await client.query(`alter function public.${identity} owner to content_schema_owner`);
    }
    ...
    const allCutover075FunctionIdentities = [
      ...
      ...cutover075ExistingCustomerCallableFunctions.map((identity) => `public.${identity}`),
      ...cutover075CustomerInternalFunctions.map((identity) => `public.${identity}`),
    ];
    assert.deepEqual(
      [...allCutover075FunctionIdentities].sort(),
      cutover075SecurityFunctions.map(({ identity }) => identity).sort(),
      "the 074 ACL harness must cover the complete 075 function security catalog",
    );
    assert.deepEqual(
      [...cutover075CustomerRelations, ...cutover075ControlRelations].sort(),
      cutover075RelationSecurityCatalog.map(({ relationName }) => relationName).sort(),
      "the 074 ACL harness must cover the complete 075 relation security catalog",
    );
    const roleNameByCatalogKey = {
    ...
    const aclDescriptorRows = [
      ...
      ...cutover075RelationSecurityCatalog.flatMap(({ relationName, grants }) => grants.map(
        ({ role, privileges }) => [
          "GRANT",
          `table:public.${relationName}|${roleNameByCatalogKey[role]}|${privileges.join(",")}`,
        ],
      )),
      ...cutover075SecurityFunctions.flatMap(({ identity, execute }) => execute.map((role) => [
        "GRANT",
        `function:${identity}|${roleNameByCatalogKey[role]}|EXECUTE`,
      ])),
    ];
    assert.ok(
      aclDescriptorRows
