  ...
  try {
    ...
    await admin.query(
      `insert into ai_content_bootstrap_state(
       ...
       ) values(true,'075-pg16-runtime-fixture',repeat('a',64),'migration_fixture',
         'schema_owner_fixture',current_user,'operator_fixture','cleanup_fixture',
         repeat('a',64),repeat('a',64),repeat('a',64),repeat('a',64),
         '{}'::jsonb,repeat('a',64),0,'{}'::jsonb,repeat('a',64))`,
    );
    ...
    await admin.query(
      `update ai_content_proposal_jobs
          set lease_started_at=now()-interval '6 minutes',
    ...
    assert.equal(postCatalog.functions.length, 37);
    ...
    await admin.query("delete from ai_content_bootstrap_state where singleton");
    await admin.query(
      `insert into ai_content_bootstrap_state(
         singleton,authorization_request_id,authorization_sha256,migration_role_name,
         schema_owner_role_name,application_role_name,operator_role_name,cleanup_role_name,
