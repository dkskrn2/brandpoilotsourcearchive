...
test("075 PostgreSQL harness applies 074 ACLs and proves atomic rollback and recovery", { timeout: 600_000 }, async (t) => {
  ...
  try {
    ...
    await assertPostSchemaTamperRejected({
      ...
      expectedError: /bootstrap_074_final_fence_security_catalog_mismatch|cutover_075_enforcement_catalog_mismatch|cutover_075_pg17_acl_compatibility_version_invalid/,
    });
    await assertPostSchemaTamperRejected({
      apply: [
...
test("074 bootstrap role authorization applies stage one then independently consumes provider evidence", async () => {
  ...
  }), /bootstrap_074_final_fence_security_catalog_mismatch|cutover_075_pg17_acl_compatibility_version_invalid/);
  ...
  for (const attack of ["function_body", "function_owner", "search_path", "security_definer", "trigger", "control_trigger_drop", "control_trigger_disable", "control_trigger_function", "control_trigger_rogue", "grant"]) {
    ...
    await assert.rejects(migrationRunner.runMigrationsWithClient({
      client, migrations: [migration], bootstrap074: { ...bootstrap, providerAttestation },
    }), /bootstrap_074_(?:fence_security|final_fence_security)_|cutover_075_pg17_acl_compatibility_version_invalid/);
    assert.equal(sealedState.attestation_consumed_at, null);
  }
  securityAttack = null;
...
test("publish calendar follows the operating suggestion and FAQ migrations deterministically", async () => {
  const loaded = await migrationRunner.loadMigrations();
  const ids = loaded.map(({ id }) => id);
  const suggestionIndex = ids.indexOf("077_content_suggestion_batches.sql");
  const faqIndex = ids.indexOf("078_faq_utterance_matching.sql");
  const calendarIndex = ids.indexOf("079_publish_calendar_runtime.sql");

  assert.notEqual(suggestionIndex, -1, "077_content_suggestion_batches.sql must exist");
  assert.notEqual(faqIndex, -1, "078_faq_utterance_matching.sql must exist");
  assert.notEqual(calendarIndex, -1, "079_publish_calendar_runtime.sql must exist");
  assert.equal(faqIndex, suggestionIndex + 1, "078 must immediately follow 077");
  assert.equal(calendarIndex, faqIndex + 1, "079 must immediately follow 078");
  assert.equal(
    migrationRunner.post075SchemaMigrationChecksums["079_publish_calendar_runtime.sql"],
    "c46ffafa578f6c1f8bb353f4e7bc94d16033416dd5a6aa730cf81119e6e6ef61",
  );
  assert.equal(loaded[calendarIndex].checksum, migrationRunner.post075SchemaMigrationChecksums[
    "079_publish_calendar_runtime.sql"
  ]);
});
...
test("onboarding product image imports follow migration 087 deterministically", async () => {
  const loaded = await migrationRunner.loadMigrations();
  const ids = loaded.map(({ id }) => id);
  const migration088Index = ids.indexOf("088_onboarding_product_image_imports.sql");

  assert.equal(
    migration088Index,
    ids.indexOf("087_ai_content_prompt_lineage_v3.sql") + 1,
  );
  assert.equal(
    migrationRunner.post075SchemaMigrationChecksums["088_onboarding_product_image_imports.sql"],
    "a83e1adecd9df47980051328c2a6bde13a3c0c58a6636f21462d77800299374c",
  );
  assert.equal(
    loaded[migration088Index].checksum,
    migrationRunner.post075SchemaMigrationChecksums["088_onboarding_product_image_imports.sql"],
  );
});
