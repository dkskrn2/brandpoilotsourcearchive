...
function serializableDraft(draft: AiContentDraft): AiContentDraft {
  return { ...draft, brief: draft.brief ? { ...draft.brief, attachments: draft.brief.attachments.map(({ file: _file, ...attachment }) => attachment) } : null };
}
...
export function AiContentWizardPage({ gateway = aiContentApiGateway, brandId = DEMO_BRAND_ID }: { gateway?: AiContentGateway; brandId?: string }) {
  ...
  const valid = state.step === 1 ? Boolean(state.draft.type) : state.step === 2 ? Boolean(state.subjectAnalysis && ["ready", "partial"].includes(state.subjectAnalysis.status)) : state.step === 3 ? Boolean(state.draft.selectedTarget && state.draft.selectedAppeal) : state.step === 5 ? Boolean(state.draft.brief?.purpose) : true;
  const actions = {
  ...
  return <div className="content ai-content-wizard">
    ...
    <div className="wizard-workspace"><AiContentWizardSteps step={state.step} draft={state.draft} actions={actions} gateway={gateway} brandId={brandId} generationId={state.generationId} analysis={state.subjectAnalysis} /></div>
    {submitError ? <p className="wizard-error" role="alert">콘텐츠 생성을 시작하지 못했습니다. 다시 시도해 주세요.</p> : null}
    <footer className="wizard-actions">{state.step > 1 ? <button type="button" className="button" onClick={state.goBack}><ChevronLeft size={17} />이전</button> : <span />}{state.step < 5 ? <button type="button" className="button primary" disabled={!valid} onClick={state.goNext}>다음<ChevronRight size={17} /></button> : <button type="button" className="button primary" disabled={!valid || submitting} onClick={() => void generate()}>{submitting ? <LoaderCircle className="inline-spinner" size={17} /> : <Sparkles size={17} />}{submitting ? "생성 요청 중" : "생성 시작"}</button>}</footer>
  </div>;
}
