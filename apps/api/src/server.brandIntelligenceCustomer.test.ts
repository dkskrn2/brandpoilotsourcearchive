...
function record(status = "queued") {
  return {
    id: analysisId, workspaceId, brandId, status, input: {
      companyName: "모종애드", ownedUrl: "https://example.com", uploadIds: [],
    },
...
function setup() {
  ...
  const intelligence = {
    ...
    cancelBrandAnalysis: vi.fn(async () => record("cancelled")),
...
describe("brand intelligence customer routes", () => {
  it("creates, reads, edits, and confirms in the authenticated brand scope", async () => {
    ...
    expect((await app.inject({
      method: "POST",
      url: `/brands/${brandId}/brand-intelligence/analyses/${analysisId}/confirm`,
      headers: auth,
      payload: { companyName: "모종애드" },
    })).statusCode).toBe(200);
    expect(intelligence.confirmBrandAnalysis).toHaveBeenCalledWith(
      expect.objectContaining({ companyName: "모종애드" }),
    );
    expect((await app.inject({
      method: "POST",
      url: `/brands/${brandId}/brand-intelligence/analyses/${analysisId}/cancel`,
      headers: auth,
    })).statusCode).toBe(200);
    await app.close();
  });
