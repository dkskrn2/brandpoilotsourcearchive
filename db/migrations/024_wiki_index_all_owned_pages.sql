...
as $$
  ...
  select
    'owned_snapshot'::text,
    snapshot.id,
    coalesce(snapshot.extracted_title, item.title, item.content_url),
  ...
  join source_content_items item
    on item.source_url_id = source.id
   and item.deleted_at is null
  join lateral (
    select latest.*
    from source_snapshots latest
    where latest.source_content_item_id = item.id
  ...
  join lateral (
    ...
    where latest.source_url_id = source.id
      and latest.source_content_item_id is null
      and latest.status = 'succeeded'
    order by latest.fetched_at desc
    limit 1
  ) snapshot on true
  where source.workspace_id = p_workspace_id
    ...
    and not exists (
      select 1
      from source_content_items item
      join source_snapshots item_snapshot
        on item_snapshot.source_content_item_id = item.id
       and item_snapshot.status = 'succeeded'
      where item.source_url_id = source.id
        and item.deleted_at is null
    );
$$;
